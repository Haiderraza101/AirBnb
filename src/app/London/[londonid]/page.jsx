...
export default async function ExperienceDetailPage(_props) {
  ...
  const london = experienceinlondon.find(
    (o) => o.id.toString() === params.londonid
  );
  if (!london) return notFound();
      ...
      <div className="mt-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center gap-6 text-sm">
          <div className="flex items-center gap-1">
            <strong className="text-lg">{london.rating}</strong>
            <span>· 16 reviews</span>
          </div>
          <div className="flex items-center gap-1">
            <strong className="text-lg">{london.price}</strong>
